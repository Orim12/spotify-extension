...
// macOS AppleScript helper functions
function getMacOSHelperScript(): string {
	return `#!/bin/bash
...
function getHelperPath(context: vscode.ExtensionContext): string {
	...
	if (platform === "darwin") {
		// macOS - use temporary helper script
		const tmpDir = require("os").tmpdir();
		return path.join(tmpDir, "spotify-helper.sh");
	} else {
		// Windows
...
class HelperClient {
	...
	private start(): void {
		...
		this.starting = true;
		let helperPath = getHelperPath(this.context);
		
		// On macOS, create the helper script from embedded code
		if (process.platform === "darwin") {
			const helperScript = getMacOSHelperScript();
			try {
...
function execHelperOnce(
	...
	return new Promise((resolve) => {
		let helperPath = getHelperPath(context);
		...
		if (process.platform === "darwin") {
			...
			try {
				...
				resolve("Failed to create helper script");
				return;
			}
		}
